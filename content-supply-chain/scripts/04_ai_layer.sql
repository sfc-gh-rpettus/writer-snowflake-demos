...
CREATE OR REPLACE CORTEX SEARCH SERVICE WRITER_SNOW_DEMO.MARKETING.CAMPAIGN_LIBRARY_SEARCH
  ...
  ATTRIBUTES CAMPAIGN_ID, CAMPAIGN_NAME, CAMPAIGN_TYPE, CHANNEL, TARGET_SEGMENT,
             ...
             SUBJECT_LINE, BODY_PREVIEW, CTA_TEXT
  WAREHOUSE = WRITER_WH
  TARGET_LAG = '1 hour'
...
CREATE OR REPLACE CORTEX SEARCH SERVICE WRITER_SNOW_DEMO.MARKETING.CAMPAIGN_BRIEFS_SEARCH
  ON SEARCH_CONTENT
  ATTRIBUTES BRIEF_ID, CAMPAIGN_ID, STATUS, CREATED_BY
  WAREHOUSE = WRITER_WH
  TARGET_LAG = '1 hour'
  COMMENT = 'Search index for campaign briefs — populated by Writer during demo'
AS
  SELECT
    BRIEF_ID, CAMPAIGN_ID, STATUS, CREATED_BY,
    COALESCE(BRIEF_CONTENT:persona_name::VARCHAR, '') || ' ' ||
    COALESCE(BRIEF_CONTENT:objective::VARCHAR, '') || ' ' ||
    COALESCE(BRIEF_CONTENT:target_audience_description::VARCHAR, '') || ' ' ||
    COALESCE(BRIEF_CONTENT:tone::VARCHAR, '') || ' ' ||
    COALESCE(BRIEF_CONTENT:brand_voice_notes::VARCHAR, '') AS SEARCH_CONTENT
          ...
          input_schema:
            ...
            properties:
              ...
              P_BRIEF_JSON:
                type: "string"
                description: >
                  Brief content as a JSON string with fields: audience_segment_id,
                  ...
                  Pass as a JSON-serialized string — the procedure parses it internally.
          ...
          input_schema:
            ...
            properties:
              ...
              P_ASSET_JSON:
                type: "string"
                description: >
                  Asset content as a JSON string with fields: campaign_id, channel
...
CREATE OR REPLACE MCP SERVER WRITER_SNOW_DEMO.MARKETING.MARKETING_MCP_SERVER
  FROM SPECIFICATION $$
    tools:
      ...
      -- (VARCHAR JSON string with fields: audience_segment_id, persona_name, objective,
      ...
      -- The procedure calls PARSE_JSON() internally — pass as a serialized JSON string.
      - name: "save-brief"
        type: "GENERIC"
        identifier: "WRITER_SNOW_DEMO.MARKETING.SAVE_BRIEF"
        title: "Save Campaign Brief"
      ...
      -- Required params: P_BRIEF_ID (VARCHAR), P_ASSET_JSON (VARCHAR JSON string with fields:
