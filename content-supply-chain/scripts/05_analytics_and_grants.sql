...
-- CAMPAIGN_BRIEFS also needs UPDATE: SAVE_BRIEF upserts via MERGE and runs
-- EXECUTE AS CALLER, so re-saving an existing BRIEF_ID hits the UPDATE branch.
...
-- ACTIVATE_SEGMENT seeds CAMPAIGN_EVENTS for the flywheel (EXECUTE AS CALLER)
-- WRITER_MARKETING_ROLE needs INSERT so the seeding works when called via MCP
GRANT INSERT ON TABLE WRITER_SNOW_DEMO.MARKETING.CAMPAIGN_EVENTS TO ROLE WRITER_MARKETING_ROLE;
